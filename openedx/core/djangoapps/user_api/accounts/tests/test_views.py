...
class TestLMSAccountRetirementPost(RetirementTestCase):
    ...
    def test_retire_user(self):
        ...
        RevisionPluginRevision.retire_user(self.test_user)
        ArticleRevision.retire_user(self.test_user)
        PendingNameChange.delete_by_user_value(self.test_user, field='user')
        PasswordHistory.retire_user(self.test_user.id)
        course_enrollments = CourseEnrollment.objects.filter(user=self.test_user)
        for enrollment in course_enrollments:
            ManualEnrollmentAudit.retire_manual_enrollments(enrollment, "retired email goes here")
